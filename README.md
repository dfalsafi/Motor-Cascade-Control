...
The electrical and mechanical equations are related based on two equal coefficients: the torque constant ($k_t$) and the back-emf constant ($k_e$). In standard SI units, $k_e = k_t$:
$$T_{em} = k_t i$$
$$e_{emf} = k_e \omega$$
